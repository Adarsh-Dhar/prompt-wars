...
export class ContrarianAgent {
    constructor(config = {}) {
        ...
        this.config = {
            ...
            apiKeys: {
                coinGecko: ((_a = config.apiKeys) === null || _a === void 0 ? void 0 : _a.coinGecko) || process.env.COINGECKO_API_KEY,
                openai: ((_b = config.apiKeys) === null || _b === void 0 ? void 0 : _b.openai) || process.env.GEMINI_API_KEY
            },
            thresholds: {
                fearGreedSellThreshold: ((_c = config.thresholds) === null || _c === void 0 ? void 0 : _c.fearGreedSellThreshold) || 60,
                extremeConditionThreshold: ((_d = config.thresholds) === null || _d === void 0 ? void 0 : _d.extremeConditionThreshold) || 80,
                bullishReinforcementThreshold: ((_e = config.thresholds) === null || _e === void 0 ? void 0 : _e.bullishReinforcementThreshold) || 70,
                bearishReinforcementThreshold: ((_f = config.thresholds) === null || _f === void 0 ? void 0 : _f.bearishReinforcementThreshold) || 30
            },
            personalitySettings: {
                smugnessLevel: ((_g = config.personalitySettings) === null || _g === void 0 ? void 0 : _g.smugnessLevel) || 8,
                personalityMode: ((_h = config.personalitySettings) === null || _h === void 0 ? void 0 : _h.personalityMode) || 'SMUG',
                catchphrases: ((_j = config.personalitySettings) === null || _j === void 0 ? void 0 : _j.catchphrases) || []
            },
            cacheSettings: {
                refreshIntervalMinutes: ((_k = config.cacheSettings) === null || _k === void 0 ? void 0 : _k.refreshIntervalMinutes) || 5,
                maxCacheAge: ((_l = config.cacheSettings) === null || _l === void 0 ? void 0 : _l.maxCacheAge) || 300000
            }
        };
    ...
    initializeModules() {
        try {
            this.sentimentFetcher = new SentimentFetcher(this.config.cacheSettings.refreshIntervalMinutes);
            this.signalGenerator = new SignalGenerator(this.config.thresholds.fearGreedSellThreshold, this.config.thresholds.extremeConditionThreshold, this.config.thresholds.bullishReinforcementThreshold, this.config.thresholds.bearishReinforcementThreshold);
            this.contrarianBrain = new ContrarianBrain(this.config.personalitySettings.smugnessLevel, this.config.personalitySettings.personalityMode);
            this.paymentService = new ContrarianPaymentService(process.env.SOLANA_RPC_ENDPOINT, parseFloat(process.env.PRICE_SOL || '0.001'), process.env.SERVER_WALLET);
            this.marketIntegration = new ContrarianMarketIntegration(this.id, process.env.FRONTEND_URL, process.env.AGENT_SERVER_URL);
            console.log(`Contrarian Agent ${this.id} initialized successfully`);
        }
        catch (error) {
            this.handleError('MODULE_INITIALIZATION_FAILED', error);
            throw error;
        }
    }
    /**
    ...
    async processMarketData(data) {
        try {
            ...
            this.handleError('MARKET_DATA_PROCESSING_FAILED', error);
        }
    }
    /**
    ...
    async generateSignal() {
        try {
            ...
            this.handleError('SIGNAL_GENERATION_FAILED', error);
            return null;
        }
    }
    /**
    ...
    async fetchMarketSentiment(tokenSymbol) {
        try {
            ...
            this.handleError('SENTIMENT_FETCH_FAILED', error);
            throw error;
        }
    }
    /**
    ...
    async generateContrarianSignal(sentimentData) {
        try {
            return this.signalGenerator.generateContrarianSignal(sentimentData, this.id, undefined // Market price can be added later
            );
        }
        catch (error) {
            this.handleError('CONTRARIAN_SIGNAL_GENERATION_FAILED', error);
            throw error;
        }
    }
    /**
    ...
    async generateSmugRant(signal) {
        try {
            ...
            this.handleError('SMUG_RANT_GENERATION_FAILED', error);
            throw error;
        }
    }
    /**
    ...
    getPersonalityResponse(context) {
        try {
            ...
            this.handleError('PERSONALITY_RESPONSE_FAILED', error);
     ...
     */
    async verifyPaymentAndDecrypt(transactionSignature, contentId, senderAddress) {
        try {
            // Verify payment
            ...
            this.handleError('PAYMENT_VERIFICATION_FAILED', error);
            throw error;
        }
    }
    /**
    ...
    generateOutput(signal) {
        ...
        return {
            agentId: this.id,
            timestamp: new Date(),
            signalType: (signal === null || signal === void 0 ? void 0 : signal.signalType) || 'BUY',
            agentState: {
            ...
            encryptedReasoning: (signal === null || signal === void 0 ? void 0 : signal.encryptedReasoning) || '',
            predictionMarket: {
                id: `market-${this.id}-${Date.now()}`,
                options: ['Knife Catcher (Rekt)', 'Alpha God (Rich)']
            },
            confidence: (signal === null || signal === void 0 ? void 0 : signal.confidence) || 75
        };
    }
    /**
     * Update agent performance after market resolution
     */
    async updatePerformance(signalId, wasCorrect) {
        try {
            if (wasCorrect) {
            ...
            this.handleError('PERFORMANCE_UPDATE_FAILED', error);
        }
    }
    /**
     ...
     */
    handleError(code, error, context) {
        const agentError = {
    ...
    async cleanup() {
        try {
            ...
            this.handleError('CLEANUP_FAILED', error);
        }
    }
    /**
     * Get signal by content ID (helper method for payment verification)
     */
    async getSignalByContentId(contentId) {
