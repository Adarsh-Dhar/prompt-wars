 ...
 */
export class ContrarianBrain {
    constructor(smugnessLevel = 8, personalityMode = 'SMUG') {
     ...
     */
    generateSmugRant(signal, sentimentData) {
     ...
     */
    getContrarianPhrases(isExtreme) {
        if (isExtreme) {
     ...
     */
    getOpeningStatement(signalType, isExtreme) {
     ...
     */
    generateMarketAnalysis(sentimentData, signal) {
     ...
     */
    generateContrarianLogic(signal, sentimentData) {
     ...
     */
    generatePersonalityCommentary(signal, isExtreme) {
     ...
     */
    generateSmugCommentary(signal, isExtreme) {
     ...
     */
    generateSuperiorCommentary(signal, isExtreme) {
        const phrases = this.getRandomPhrases(isExtreme, 2);
        if (signal.signalType === 'BUY') {
     ...
     */
    generateCynicalCommentary(signal, isExtreme) {
        const phrases = this.getRandomPhrases(isExtreme, 2);
        if (signal.signalType === 'BUY') {
     ...
     */
    generateClosingStatement(signal, isExtreme) {
     ...
     */
    getRandomPhrases(isExtreme, count) {
     ...
     */
    adjustSmugnessLevel(newLevel) {
        // Clamp between 1-10
        const clampedLevel = Math.max(1, Math.min(10, newLevel));
        this.smugnessLevel = clampedLevel;
    }
    /**
