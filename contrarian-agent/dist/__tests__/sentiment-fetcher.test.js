...
describe('Sentiment Fetcher Property Tests', () => {
    ...
    test('Property 2: Token sentiment retrieval - CoinGecko API', () => {
        fc.assert(fc.property(fc.constantFrom('BTC', 'ETH', 'SOL', 'ADA', 'DOT'), // token symbols
    ...
    test('Property 3: Network error resilience - Retry logic', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 3 }), // number of failures before success
    ...
    test('Property 4: Sentiment data validation - Fear & Greed range', () => {
        fc.assert(fc.property(fc.integer({ min: -100, max: 200 }), // test values outside valid range too
        (testValue) => {
            const validData = {
                value: testValue,
    ...
    test('Property 24: Cached data fallback - API unavailable', () => {
        fc.assert(fc.property(fc.integer({ min: 0, max: 100 }), // cached value
        async (cachedValue) => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Using stale cached Fear & Greed data'));
    ...
    test('validateSentimentData - various input types', () => {
        fc.assert(fc.property(fc.oneof(fc.constant(null), fc.constant(undefined), fc.string(), fc.integer(), fc.record({
