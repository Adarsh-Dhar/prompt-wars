...
describe('Contrarian Brain Property Tests', () => {
    ...
    test('Property 13: Personality consistency - Arrogant tone with contrarian phrases', () => {
        ...
        (signalType, fearGreedValue, classification, confidence, isExtremeCondition) => {
            ...
            const hasContrarianPhrase = contrarianPhrases.some(phrase => rant.toLowerCase().includes(phrase.toLowerCase()));
            ...
            const hasArrogantTone = arrogantIndicators.some(indicator => rant.toLowerCase().includes(indicator.toLowerCase()));
    ...
    test('Property 14: Extreme condition catchphrases - Specific phrases for extreme conditions', () => {
        fc.assert(fc.property(fc.constantFrom('BUY', 'SELL'), // signal type
        fc.integer({ min: 0, max: 19 }).chain(low => fc.constantFrom(low, 100 - low) // Extreme values (very low or very high)
        ...
        (signalType, extremeFearGreedValue, confidence) => {
            ...
            if (isExtremeCondition) {
                ...
                const hasExtremePhrase = extremePhrases.some(phrase => rant.toUpperCase().includes(phrase));
    ...
    test('getPersonalityResponse - consistent personality across contexts', () => {
        fc.assert(fc.property(fc.string({ minLength: 1, maxLength: 50 }).filter(s => s.trim().length > 0), (context) => {
            ...
            const hasPersonality = personalityIndicators.some(indicator => response.toLowerCase().includes(indicator));
            expect(hasPersonality).toBe(true);
        }), { numRuns: 100 });
    });
    test('getContrarianPhrases - different phrases for extreme vs normal conditions', () => {
        fc.assert(fc.property(fc.boolean(), (isExtreme) => {
            ...
            if (isExtreme) {
                const hasIntensePhrases = phrases.some(phrase => phrase.includes('MAXIMUM') ||
    ...
    test('Different personality modes generate different commentary styles', () => {
        const modes = ['SMUG', 'SUPERIOR', 'CYNICAL'];
        fc.assert(fc.property(fc.constantFrom(...modes), fc.constantFrom('BUY', 'SELL'), fc.integer({ min: 0, max: 100 }), (personalityMode, signalType, fearGreedValue) => {
            ...
            switch (personalityMode) {
                case 'SUPERIOR':
                    ...
                    const hasSuperiorTone = superiorWords.some(word => rant.toLowerCase().includes(word));
                    expect(hasSuperiorTone).toBe(true);
                    break;
                case 'CYNICAL':
                    const cynicalWords = ['another', 'never learn', 'same', 'cycle', 'game'];
                    const hasCynicalTone = cynicalWords.some(word => rant.toLowerCase().includes(word));
                    expect(hasCynicalTone).toBe(true);
                    break;
                case 'SMUG':
                    const smugWords = ['while', 'thanks', 'i\'ll be', 'as usual', 'this is why'];
                    const hasSmugTone = smugWords.some(word => rant.toLowerCase().includes(word));
    ...
    test('Smugness level affects personality configuration', () => {
        fc.assert(fc.property(fc.integer({ min: 1, max: 10 }), (smugnessLevel) => {
    ...
    test('Community sentiment affects rant content', () => {
        fc.assert(fc.property(fc.constantFrom('BUY', 'SELL'), fc.integer({ min: 0, max: 100 }), // community bullish percentage
        fc.integer({ min: 0, max: 100 }), // fear greed value
