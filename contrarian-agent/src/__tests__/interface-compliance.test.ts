...
import { IAgent, IContrarianAgent, ContrarianSignal, MarketData, SentimentData } from '../types/index.js';

// Mock implementation for testing interface compliance
class MockContrarianAgent implements IContrarianAgent {
  id: string;

  constructor(id: string = 'test-contrarian-agent') {
    this.id = id;
  }
...
describe('Interface Compliance Property Tests', () => {
  ...
  test('Property 17: Interface pattern compliance - IAgent interface', () => {
    fc.assert(fc.property(
      fc.string({ minLength: 1, maxLength: 50 }).filter(s => s.trim().length > 0), // agent id
      async (agentId) => {
        const agent: IAgent = new MockContrarianAgent(agentId);
  ...
  test('Property 17: Interface pattern compliance - IContrarianAgent interface', () => {
    fc.assert(fc.property(
      ...
      async (agentId, tokenSymbol) => {
        const agent: IContrarianAgent = new MockContrarianAgent(agentId);
