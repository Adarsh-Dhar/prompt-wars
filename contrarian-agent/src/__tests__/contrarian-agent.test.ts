...
describe('Contrarian Agent Property Tests', () => {
  ...
  test('Property 18: Frontend protocol compliance - Established integration protocols', () => {
    fc.assert(fc.property(
      fc.string({ minLength: 1, maxLength: 10 }).filter(s => s.trim().length > 0), // token symbol
  ...
  test('Property 19: Error handling consistency - Existing error mechanisms', () => {
    fc.assert(fc.property(
      fc.string({ minLength: 1, maxLength: 100 }), // context string
      (context) => {
  ...
  test('Performance tracking accuracy', () => {
    fc.assert(fc.property(
      fc.array(fc.boolean(), { minLength: 1, maxLength: 20 }), // array of correct/incorrect results
      async (results) => {
        const testAgent = new ContrarianAgent();
  ...
  test('Configuration validation and defaults', () => {
    fc.assert(fc.property(
      ...
      (partialConfig) => {
        ...
        expect(config.thresholds.fearGreedSellThreshold).toBe(
          partialConfig.fearGreedSellThreshold || 60
        );
        expect(config.thresholds.extremeConditionThreshold).toBe(
          partialConfig.extremeConditionThreshold || 80
        );
        expect(config.personalitySettings.smugnessLevel).toBe(
          partialConfig.smugnessLevel || 8
        );
        expect(config.cacheSettings.refreshIntervalMinutes).toBe(
          partialConfig.refreshIntervalMinutes || 5
        );
  ...
  test('Health check comprehensive status reporting', async () => {
    ...
    Object.values(healthStatus.details).forEach((component: any) => {
