...
import { SentimentData, ContrarianSignal, MarketData } from '../types/index.js';
...
describe('Unit Tests - Core Functionality', () => {
  ...
  describe('SignalGenerator', () => {
    ...
    test('contrarian logic boundary conditions', () => {
      ...
      const greedySentiment: SentimentData = {
  ...
  describe('ContrarianBrain', () => {
    ...
    test('contrarian phrases for different conditions', () => {
      ...
      const hasIntensePhrase = extremePhrases.some(phrase => 
        phrase.includes('MAXIMUM') || phrase.includes('EXTREME') || phrase === phrase.toUpperCase()
      );
  ...
  describe('ContrarianPaymentService', () => {
    let paymentService: ContrarianPaymentService;

    beforeEach(() => {
      paymentService = new ContrarianPaymentService(
