...
describe('Signal Generator Property Tests', () => {
  ...
  test('Property 6: Contrarian sell logic - Fear & Greed > 60 generates SELL', () => {
    fc.assert(fc.property(
      fc.integer({ min: 61, max: 100 }), // Fear & Greed values > 60
  ...
  test('Property 7: Contrarian buy logic - Fear & Greed ≤ 60 generates BUY', () => {
    fc.assert(fc.property(
      fc.integer({ min: 0, max: 60 }), // Fear & Greed values ≤ 60
  ...
  test('Property 8: Bullish sentiment reinforcement - >70% bullish reinforces SELL', () => {
    fc.assert(fc.property(
      fc.integer({ min: 61, max: 100 }), // Fear & Greed > 60 (SELL territory)
      fc.integer({ min: 71, max: 100 }), // Community bullish > 70%
      (fearGreedValue, bullishPercentage) => {
        const sentimentDataWithoutCommunity: SentimentData = {
          fearGreedIndex: {
            value: fearGreedValue,
        ...
        const sentimentDataWithCommunity: SentimentData = {
          fearGreedIndex: {
            value: fearGreedValue,
            classification: 'Greed',
            timestamp: new Date()
  ...
  test('Property 9: Bearish sentiment reinforcement - <30% bullish reinforces BUY', () => {
    fc.assert(fc.property(
      fc.integer({ min: 0, max: 60 }), // Fear & Greed ≤ 60 (BUY territory)
  ...
  test('Property 21: Extreme fear signal intensity - Fear > 80 triggers aggressive BUY', () => {
    fc.assert(fc.property(
      fc.integer({ min: 0, max: 19 }), // Extreme fear (< 20, which is > 80 on fear scale)
      (fearGreedValue) => {
        const sentimentData: SentimentData = {
          fearGreedIndex: {
            value: fearGreedValue,
  ...
  test('Property 22: Extreme greed signal intensity - Greed > 80 triggers aggressive SELL', () => {
    fc.assert(fc.property(
      fc.integer({ min: 81, max: 100 }), // Extreme greed (> 80)
      (fearGreedValue) => {
        const sentimentData: SentimentData = {
          fearGreedIndex: {
            value: fearGreedValue,
  ...
  test('Property 23: Confidence adjustment in extremes - Extreme conditions boost confidence', () => {
    fc.assert(fc.property(
      fc.integer({ min: 30, max: 70 }), // Normal conditions
      fc.integer({ min: 0, max: 19 }).chain(normal => 
        fc.constantFrom(normal, 100 - normal) // Extreme conditions (very low or very high)
      ),
      (normalValue, extremeValue) => {
        const normalSentiment: SentimentData = {
          fearGreedIndex: {
            value: normalValue,
  ...
  test('validateSignalParameters - input validation', () => {
    fc.assert(fc.property(
      ...
      (fearGreed, community) => {
        const isValid = signalGenerator.validateSignalParameters(fearGreed as any, community as any);
