...
import { ContrarianSignal } from '../types/index.js';

describe('Market Integration Property Tests', () => {
  let marketIntegration: ContrarianMarketIntegration;

  beforeEach(() => {
    marketIntegration = new ContrarianMarketIntegration(
  ...
  test('Performance tracking maintains correct statistics', () => {
    fc.assert(fc.property(
      fc.array(fc.constantFrom('KNIFE_CATCHER', 'ALPHA_GOD'), { minLength: 1, maxLength: 20 }),
      async (outcomes) => {
        ...
        for (let i = 0; i < outcomes.length; i++) {
          ...
          (integration as any).activeMarkets.set(`market-${i}`, mockMarket);
  ...
  test('Market data structure consistency', () => {
    fc.assert(fc.property(
      fc.string({ minLength: 1, maxLength: 50 }), // market ID
  ...
  test('Signal preview generation', () => {
    fc.assert(fc.property(
      ...
      fc.boolean(),
      (signalType, confidence, fearGreedValue, isExtremeCondition) => {
        const signal: ContrarianSignal = {
          id: 'test-signal',
          agentId: 'test-agent',
          signalType: signalType,
        ...
        const integration = marketIntegration as any;
  ...
  test('Agent registration data structure', () => {
    fc.assert(fc.property(
      ...
      (agentId, serverUrl) => {
        ...
        expect((integration as any).agentId).toBe(agentId);
        expect((integration as any).serverUrl).toBe(serverUrl);
        expect((integration as any).frontendUrl).toBe('http://localhost:3000');
  ...
  test('Market cleanup functionality', () => {
    fc.assert(fc.property(
      fc.integer({ min: 1, max: 30 }), // days old
